...
from .config_manager import ConfigManager, get_converter
...
def merge_program_config(
        ...
        *,
        ...
        games_sorting_mode: Optional[GamesSortingMode]) -> ProgramConfig:
    """
    Merge `program_config` with CLI options. Any specified CLI options will
    override the existing values in `program_config`.
    """
    converter = get_converter()
    default_locale_structured = converter.structure(
        default_locale, OneLauncherLocale) if default_locale else None

    return attrs.evolve(
        program_config,
        default_locale=(default_locale_structured or
                        program_config.default_locale),
        always_use_default_locale_for_ui=(
            always_use_default_locale_for_ui if
            always_use_default_locale_for_ui is not None else
            program_config.always_use_default_locale_for_ui),
        save_accounts=(save_accounts if save_accounts is not None else
                       program_config.save_accounts),
        save_accounts_passwords=(save_accounts_passwords if
                                 save_accounts_passwords is not None else
                                 program_config.save_accounts_passwords),
        games_sorting_mode=(games_sorting_mode or
...
def merge_game_config(
        ...
        *,
        ...
        wine_debug_level: str | None) -> GameConfig:
    """
    Merge `game_config` with CLI options. Any specified CLI options will
    override the existing values in `game_config`.
    """
    converter = get_converter()
    locale_structured = converter.structure(
        locale, OneLauncherLocale) if locale else None

    wine_section = attrs.evolve(
        game_config.wine,
        builtin_prefix_enabled=(
            builtin_prefix_enabled if
            builtin_prefix_enabled is not None else
            game_config.wine.builtin_prefix_enabled),
        user_wine_executable_path=(
            user_wine_executable_path if
            user_wine_executable_path is not None else
            game_config.wine.user_wine_executable_path),
        user_prefix_path=(
            user_prefix_path if
            user_prefix_path is not None else
            game_config.wine.user_prefix_path),
        debug_level=(
            wine_debug_level if
            wine_debug_level is not None else
            game_config.wine.debug_level))
    return attrs.evolve(
        game_config,
        game_directory=(CaseInsensitiveAbsolutePath(game_directory) if
                        game_directory is not None else
                        game_config.game_directory),
        locale=(locale_structured if
                locale_structured is not None else game_config.locale),
        client_type=(
            client_type if
            client_type is not None else game_config.client_type),
        high_res_enabled=(high_res_enabled if
                          high_res_enabled is not None else
                          game_config.high_res_enabled),
        standard_game_launcher_filename=(
            standard_game_launcher_filename if
            standard_game_launcher_filename is not None else
            game_config.standard_game_launcher_filename),
        patch_client_filename=(patch_client_filename if
                               patch_client_filename is not None else
                               game_config.patch_client_filename),
        newsfeed=(newsfeed if
                  newsfeed is not None else
                  game_config.newsfeed),
        wine=wine_section
    )
...
def merge_accounts_config(
        ...
        *,
        ...
        last_used_world_name: str | None) -> GameAccountsConfig:
    """
    ...
    evolved_account = attrs.evolve(
        account,
        display_name=(display_name if
                      display_name is not None else
                      account.display_name),
        last_used_world_name=(
            last_used_world_name if
            last_used_world_name is not None else
...
def _parse_game_arg(
        game_arg: str, config_manager: ConfigManager) -> UUID:
    """
    Raises:
        typer.BadParameter
    """
    ...
    if game_arg not in tuple(GameOptions):
        ...
        if game_uuid not in game_uuids:
            raise typer.BadParameter(
                message="Provided game UUID does not exist",
                param_hint="--game")
        return game_uuid
    ...
    for game_uuid in game_uuids:
        game_config = config_manager.read_game_config_file(
            game_uuid=game_uuid)
        if (game_config.game_type == game_type and
                game_config.is_preview_client == is_preview):
            return game_uuid
    raise typer.BadParameter(
        message=f"No {game_arg} games exist",
        param_hint="--game")
...
def _get_starting_game_uuid(
        game_arg: str | None,
        config_manager: ConfigManager) -> UUID:
    try:
...
def _complete_username_arg(
        incomplete: str,
        context: typer.Context) -> Iterator[str]:
    game_arg: str | None = context.params.get("game")
    if not game_arg:
        return
    config_manager = ConfigManager(lambda c: c, lambda c: c, lambda c: c)
    try:
        game_uuid = _parse_game_arg(
            game_arg=game_arg,
            config_manager=config_manager)
    except typer.BadParameter:
        return
    usernames = (
        account.username for account in
        config_manager.read_game_accounts_config_file(
            game_uuid))
...
ProgramOption = partial(
    typer.Option,
    show_default=False,
    rich_help_panel="Program Options")

GameOption = partial(
    typer.Option,
    show_default=False,
    rich_help_panel="Game Options")
...
AccountOption = partial(
    typer.Option,
    show_default=False,
    rich_help_panel="Game Account Options")


def get_help(field_name: str, /,
             attrs_class: type[attrs.AttrsInstance]) -> str | None:
    return ConfigFieldMetadata.from_field_name(
        field_name=field_name,
        attrs_class=attrs_class).help
...
def main(version: Annotated[
        ...
        typer.Option("--version",
                     ...
                     is_eager=True,
                     callback=version_calback)] = False,
         # Program options
         default_locale: Annotated[
             Optional[str],
             ProgramOption(help=prog_help("default_locale"))] = None,
         always_use_default_locale_for_ui: Annotated[
             Optional[bool],
             ProgramOption(help=prog_help(
                 "always_use_default_locale_for_ui"))] = None,
         save_accounts: Annotated[
             Optional[bool],
             ProgramOption(help=prog_help("save_accounts"))] = None,
         save_accounts_passwords: Annotated[
             Optional[bool],
             ProgramOption(help=prog_help("save_accounts_passwords"))] = None,
         games_sorting_mode: Annotated[
             Optional[GamesSortingMode],
             ProgramOption(help=prog_help("games_sorting_mode"))] = None,
         # Game options
         game: Annotated[
             Optional[str],
             GameOption(help=("Which game to load. ([yellow]"
                              f"{', '.join(GameOptions)}, or a game UUID)"),
                        parser=game_type_or_uuid,
                        autocompletion=_complete_game_arg)] = None,
         game_directory: Annotated[
             Optional[Path],
             GameOption(help=game_help("game_directory"),
                        exists=True,
                        file_okay=False,
                        dir_okay=True,
         ...
         locale: Annotated[
             Optional[str],
             GameOption(help=game_help("locale"))] = None,
         client_type: Annotated[
             Optional[ClientType],
             GameOption(help=game_help("client_type"),
                        case_sensitive=False)] = None,
         high_res_enabled: Annotated[
             Optional[bool],
             GameOption(help=game_help("high_res_enabled"))] = None,
         standard_game_launcher_filename: Annotated[
             Optional[str],
             GameOption(
                 help=game_help("standard_game_launcher_filename"))] = None,
         patch_client_filename: Annotated[
             Optional[str],
             GameOption(help=game_help("patch_client_filename"))] = None,
         newsfeed: Annotated[
             Optional[str],
             GameOption(help=game_help("newsfeed"))] = None,
         # Game WINE options
         builtin_prefix_enabled: Annotated[
             Optional[bool],
             WineOption(help=wine_help("builtin_prefix_enabled"))] = None,
         user_wine_executable_path: Annotated[
             Optional[Path],
             WineOption(help=wine_help("user_wine_executable_path"),
                        exists=True,
                        file_okay=True,
                        dir_okay=False,
                        resolve_path=True)] = None,
         user_prefix_path: Annotated[
             Optional[Path],
             WineOption(help=wine_help("user_prefix_path"),
                        exists=True,
                        file_okay=False,
                        dir_okay=True,
                        resolve_path=True)] = None,
         wine_debug_level: Annotated[
             Optional[str],
             WineOption(help=wine_help("debug_level"))] = None,
         # Account options
         username: Annotated[
             Optional[str],
             AccountOption(help=account_help("username"),
                           autocompletion=_complete_username_arg)] = None,
         display_name: Annotated[
             Optional[str],
             AccountOption(help=account_help("display_name"))] = None,
         last_used_world_name: Annotated[
             Optional[str],
             AccountOption(help=account_help("last_used_world_name"))] = None
         ) -> None:
    ...
    game_uuid = _get_starting_game_uuid(
        game_arg=game, config_manager=config_manager)
