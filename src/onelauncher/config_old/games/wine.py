...
class WineConfig:
    ...
    def from_config_dict(cls, config_dict: dict[str, Any]) -> Self:
        ...
        if "user_wine_executable_path" in config_dict:
            user_wine_executable_path = Path(config_dict["user_wine_executable_path"])
        ...
        return cls(
            config_dict.get("builtin_prefix_enabled", True),
...
def get_config_dict_from_wine_config(wine_config: WineConfig) -> dict[str, Any]:
    config_dict: dict[str, Any] = {
        ...
        if wine_config.user_wine_executable_path
        else None,
        "user_prefix_path": str(wine_config.user_prefix_path)
        if wine_config.user_prefix_path
        else None,
...
def get_config_from_wine_environment(wine_env: WineEnvironment) -> dict[str, Any]:
    config_dict: dict[str, Any] = {
...
def get_wine_environment_from_config(wine_config: dict[str, Any]) -> WineEnvironment:
    # Deal with Paths
    user_wine_executable_path = None
    if "user_wine_executable_path" in wine_config:
        user_wine_executable_path = Path(wine_config["user_wine_executable_path"])
    ...
    return WineEnvironment(
        wine_config.get("builtin_prefix_enabled", True),
        user_wine_executable_path,
        user_prefix_path,
...
def get_wine_management_from_wine_config(wine_config: WineConfig) -> WineManagement:
...
def get_wine_management_from_game(game: Game) -> WineManagement:
    return get_wine_management_from_wine_config(get_wine_config_from_game(game))
...
def get_wine_config_from_game(game: Game) -> WineConfig:
    config_dict = games_config.get_game_config_section(game.uuid, CONFIG_SECTION_NAME)
...
def save_wine_environment(game: Game, wine_env: WineEnvironment) -> None:
    ...
    games_config.save_game_config_section(game.uuid, CONFIG_SECTION_NAME, config)


def save_wine_config(game: Game, wine_config: WineConfig) -> None:
    config_dict = get_config_dict_from_wine_config(wine_config)
    games_config.save_game_config_section(game.uuid, CONFIG_SECTION_NAME, config_dict)
